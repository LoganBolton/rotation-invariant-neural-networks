...
```bash
uv run python benchmarks/run_models/train.py --model hiphop --k 4 --epochs 5000 --l-max 2 --n-max 3
```

To try HIP-NN-TS with vector features:

```bash
uv run python benchmarks/run_models/train.py --model hipnnvec --k 4 --epochs 5000
```
...
```bash
uv run python benchmarks/run_models/train.py --k 4 --n-interaction-layers 3 --dist-hard-max 6.5
```
...
```bash
uv run python benchmarks/run_models/train.py --k 4 --dist-hard-max 30
```
...
```bash
uv run python benchmarks/run_models/sweep.py --model hiphop --k 3 4 5 --epochs 2000 --seeds 0 1 2 --interaction-layers 3 4 --hard-cutoffs 6.5 30
```

or `--model hipnnvec`:

```bash
uv run python benchmarks/run_models/sweep.py --model hipnnvec --k 3 4 5 --epochs 2000 --seeds 0 1 2 --interaction-layers 3 4 --hard-cutoffs 6.5 30
```
